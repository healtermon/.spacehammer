...
;;; AppToggle Spoon
;; I use this because it works; the only other thing I tried, this alfred workflow, lags:https://github.com/shuaibird/toggle-visibility-alfred-workflow
;; use the command line to find the bundle ID of apps, since the auto-finding is not so reliable:
...
;; translation of the above lua code to fennel:
(local app_mod ["alt" "shift" "cmd" "ctrl"])
(local apps
       [[app_mod "1" "Whatsapp"]
        [app_mod "2" "Telegram"]
        [app_mod "3" "jp.naver.line.mac"]
        [app_mod "4" "Discord"]
        [app_mod "5" "WeChat" ]
        ;; [app_mod "a" "iTerm"]
        [app_mod "e" "com.electron.realtimeboard"]
        ;; [app_mod "w" "Finder"] ; too messy; shows all windows
  ;; { app_mod, "b", "Finder" },
  ;; { app_mod, "d", "Dash" },
  ;; { app_mod, "space", "iTerm" },
  ;; { { "alt", "shift" }, "c", "Calendar" }
...
;;; Toggle Wifi function & Setup
;; URL: https://www.darklaunch.com/toggle-wifi-enabled-hammerspoon.html
...
;; translation of the above lua code to fennel:
(fn toggleWifiEnabled []
  (fn []
    (if (= (hs.network.interfaceDetails) nil)
        (do
          (print "toggleWifiEnabled")
          (hs.wifi.setPower true)
          (print "network turned on"))
...
(hs.hotkey.bind app_mod "'" (toggleWifiEnabled))    

;;; Exports (a table containing what you want to expose, check other fennel files for more examples)
